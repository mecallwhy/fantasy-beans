...
export const PlayerBean = (props) => {
  ...
  const playersOfSamePosition = player && theSquad.filter(singlePlayer => singlePlayer.playerData.position === player.playerData.position);
  ...
  const switchPlayers = () => {
    const playerToSwitch = theSquad.find(singlePlayer => singlePlayer.playerData.id === playerToSwitchId);
    const temporarySlot = player.role;
  ...
  if(isUnusedBean)
    ...
    const className1 = "player-bean"
    const className2 = "player-bean-" + player.playerData.club.toLowerCase().slice(0, 3)
    const className3 = player.playerData.className
    
    return (
      <div className={className1 + " " + className2 + " " + className3}>
        <span
        ...
        <button
          ...
          onClick={prepareToSwitch}>
          <img className="switch-img-small" src={recycle2} alt=""></img>
        </button>
        <button 
          className="player-bean-btn-switch-big" 
