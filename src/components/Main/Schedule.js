    ...
    return (
        <div id={id}>
          ...
          <div id="schedule-matchday">
            ...
            {matchdayIndex > 0 &&
            <button
              id="schedule-matchday-prev-button"
              onClick={()=>{setMatchdayIndex(matchdayIndex-1)}}>
              <img src={leftArrow} alt="left"></img>
            </button>}
            {matchdayIndex < matchdaysNumber-1 &&
            <button
              id="schedule-matchday-next-button"
              onClick={()=>{setMatchdayIndex(matchdayIndex+1)}}>
          ...
          {schedule &&
            <Matchday
              league={league}
              season={season}
              matchday={schedule[matchdayIndex]}/>}
        </div>
    )
}
